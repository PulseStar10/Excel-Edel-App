...
with st.sidebar:
  st.header("🔐 Secure Account Access")
    ...
    if auth_tab == "Sign Up":
      ...
      if st.button("Login", type="primary"):
        ...
        if clean_user in users_db and users_db[clean_user]["password"] == hash_password(
          ...
          st.session_state.records = users_db[clean_user].get("records", [])
  ...
  if st.button("🗑️ Clear My Saved Data", type="secondary"):
    ...
    if st.session_state.username in users_db:
      users_db[st.session_state.username]["records"] = []
      save_users(users_db)
...
def save_user_records():
  users_db = load_users()
  if st.session_state.username in users_db:
    users_db[st.session_state.username]["records"] = st.session_state.records
    save_users(users_db)
...
st.markdown(
    "Your records are securely tied to your password-protected account and will"
    " never disappear on reload!"
)
  ...
  if "excel_path" in st.session_state and os.path.exists(
    ...
    with open(st.session_state.excel_path, "rb") as f:
      st.download_button(
          ...
          data=f,
          file_name=f"Conveyance_{datetime.date.today().strftime('%Y%m%d')}.xlsx",
          mime=(
